<?php
        /*if(empty($_POST['username'])||strlen($_POST['username'])>11){
            echo "<h3>Enter name properly</h3>";
            echo "<br><a href=registration.php>retry</a>";
	}
        else if(strlen($_POST['newpass'])<6||strlen($_POST['newpass'])>10||strcmp($_POST['newpass'],$_POST['reenternewpass'])!=0){
        ...
        */
        ...
        if(!empty($_FILES["student_image"]["name"]))
        {
            ...
            if (!file_exists('uploads/'.$student_email)) 
            {
            ...
            if(! (move_uploaded_file($_FILES["student_image"]["tmp_name"], $target_location))|($_FILES["student_image"]["size"]>300000)|(strcmp($_FILES["student_image"]["type"],'image/png')!=0)){
                echo "Error0101: ".$_FILES["student_image"]["tmp_name"].", ".$target_location. $_FILES["student_image"]["error"] . "<br>size?:".$_FILES["student_image"]["size"]." png?:".strcmp($_FILES["student_image"]["type"],'image/png');
                die();
            }
            else
            {
                ...
                try{
                    ...
                    . "(student_first_name,student_middle_name,student_last_name,student_dob,student_gender,student_semester,student_course,student_address,student_address2,student_city,student_state,student_zip,student_moblie_no,student_id_no) "
                    . "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                    ...
                    $prepared_sql->execute(array($_POST["student_first_name"],$_POST["student_middle_name"],$_POST["student_last_name"],$_POST["student_gender"],$date,$date,$_POST["student_semester"],$_POST["student_course"],$_POST["student_address"],$_POST["student_address2"],$_POST["student_city"],$_POST["student_state"],$_POST["student_zip"],$_POST["student_mobile_no"],$_POST["student_id_no"]));
